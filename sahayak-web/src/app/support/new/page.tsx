...
export default function NewTicketPage() {
  ...
  const submit = async () => {
    ...
    try {
      const res = await create.mutateAsync({ subject: subject.trim(), categoryId: categoryId || undefined, message: message.trim() });
    ...
    <div className="mx-auto max-w-2xl card">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 flex gap-2">
          ...
          <button className="btn-outline" onClick={()=>history.back()}>Cancel</button>
        </div>
      </div>
    </div>
  );
